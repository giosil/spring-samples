...
(function (APP) {
    var WUtil = WUX.WUtil;
    var GUIChat = /** @class */ (function (_super) {
        __extends(GUIChat, _super);
        function GUIChat() {
            var _this = _super.call(this, '*', 'GUIChat') || this;
            _this.defModel = 'ai/llama3.2:1B-Q4_0';
            _this.defMaxTokens = 1000;
            _this.defTemperature = 0.7;
            return _this;
        }
        GUIChat.prototype.render = function () {
            var _this = this;
            this.brcr = new APP.Breadcrumb();
            this.brcr.add('Chat');
            var models = ['', 'ai/llama3.2:1B-Q4_0'];
        ...
        GUIChat.prototype.doSend = function () {
            var values = this.form.getState();
            var model = values['model'];
            var maxT = values['maxTokens'];
            var temp = values['temperature'];
            var topP = values['topP'];
            var freq = values['freqPenalty'];
            var pres = values['presPenalty'];
            if (maxT != null)
                maxT = parseInt(maxT);
            if (temp != null)
            ...
            var usrMessage = WUtil.getString(values, 'usrMessage');
            if (!usrMessage) {
                APP.showWarning("Scrivere un prompt da inviare");
                return;
            }
            var heOut = document.getElementById(this.idOut);
            if (!heOut) {
                APP.showWarning("Elemento con id '" + this.idOut + "' non trovato.");
                return;
            }
            heOut.textContent = '';
            var sysMessage = WUtil.getString(values, 'sysMessage');
            var assMessage = WUtil.getString(values, 'assMessage');
            var messages = [];
            if (sysMessage) {
            ...
            fetch('http://localhost:8081/chat/completions', {
                ...
                if (!res.ok) {
                    ...
                    APP.showError('Errore HTTP ' + res.status, 'Errore chiamata');
                    return;
                }
                ...
                function read() {
                    reader.read().then(function (_a) {
                        ...
                        if (done)
                            return;
                        var chunk = tdec.decode(value, { stream: true });
        ...
        GUIChat.prototype.doReset = function () {
            var heOut = document.getElementById(this.idOut);
            if (heOut)
                heOut.textContent = '';
            this.form.clear();
